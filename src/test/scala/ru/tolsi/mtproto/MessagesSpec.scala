...
import ru.tolsi.mtproto.messages.{EncyptedPqInnerData, PqInnerData, ReqDHParams, ReqPq}
...
class MessagesSpec extends WordSpec with Matchers {
  ...
  "ReqDHParams codec" should {
    "encode a valid message" in {
      ...
      val req = ReqDHParams(nonce, serverNonce, p, q, publicRSAKeys.head.fingerprint, EncyptedPqInnerData.encrypt(pqInnerData, publicRSAKeys.head))
      val blob =
