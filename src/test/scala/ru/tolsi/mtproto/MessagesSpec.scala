...
import ru.tolsi.mtproto.crypto.crypto.publicRSAKeys
...
class MessagesSpec extends WordSpec with Matchers {
  ...
  "ReqDHParams codec" should {
    "encode a valid message" in {
      ...
      val req = ReqDHParams(nonce, serverNonce, p, q, rsaKey.fingerprint, EncyptedPqInnerData.encrypt(pqInnerData, rsaKey))
      val blob =
